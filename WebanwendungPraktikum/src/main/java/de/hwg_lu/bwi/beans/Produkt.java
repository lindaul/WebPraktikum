...
public class Produkt {
	...
	public Produkt(int produktId, String produktName, String produktNummer, String beschreibungProdukt, double preis,
			int bestand, double rabatt, String kategorie, String marke) {
		super();
	...
	public void updateProduktDB(int produktId, String produktNummer, String produktName, String beschreibungProdukt,
		...
		String sql = "update produkt set produktNummer =?," + "produktName =?," + "beschreibungProdukt=?," + "preis =?,"
				+ "bestand =?," + "rabatt=?," + "kategorie=?," + "marke=?" + ") where produktid = ?";
		System.out.println(sql);
		PreparedStatement prep = this.dbConn.prepareStatement(sql);
	...
	public void getAllProduktDB() throws SQLException {
		...
		String sql = "select " + "produktid," + "produktname," + "produktnummer," + "beschreibungprodukt," + "preis, "
				+ "bestand, " + "rabatt, " + "kategorie," + "marke " + "from bwi520_633040_634997.produkt";
		System.out.println(sql);
		...
		while (dbRes.next()) {
			this.produktlist.add(new Produkt(dbRes.getInt("produktid"), dbRes.getString("produktname"),
					dbRes.getString("produktnummer"), dbRes.getString("beschreibungprodukt"), dbRes.getDouble("preis"),
					dbRes.getInt("bestand"), dbRes.getDouble("rabatt"), dbRes.getString("kategorie"),
	...
	public void getOneProduktDB(int produktId) throws SQLException {
		...
		String sql = "select " + "produktid, "

				+ "produktname," + "produktnummer," + "beschreibungprodukt," + "preis, " + "bestand, " + "rabatt, "
				+ "kategorie," + "marke " + "from bwi520_633040_634997.produkt" + " where produktid =" + produktId;
		System.out.println(sql);
		ResultSet dbRes = new PostgreSQLAccess().getConnection().prepareStatement(sql).executeQuery();
	...
	public Produkt getOneProduktFromDB(int produktId) throws SQLException {
		...
		while (dbRes.next()) {
			...
			produktNew = new Produkt(dbRes.getInt("produktid"), dbRes.getString("produktname"),
					dbRes.getString("produktnummer"), dbRes.getString("beschreibungprodukt"), dbRes.getDouble("preis"),
					dbRes.getInt("bestand"), dbRes.getDouble("rabatt"), dbRes.getString("kategorie"),
					dbRes.getString("marke")
	...
	public String toString() {
		return "Produkt [produktId=" + produktId + ", produktName=" + produktName + ", produktNummer=" + produktNummer
				+ ", beschreibungProdukt=" + beschreibungProdukt + ", preis=" + preis + ", bestand=" + bestand
				+ ", rabatt=" + rabatt + ", kategorie=" + kategorie + ", marke=" + marke + ", produktlist="
				+ produktlist + "]";
	}
	...
	public String getProductCard() throws SQLException {
		...
		if(!this.produktCategorieList.isEmpty()) {
			
			
			
			for (Produkt produkt : produktCategorieList) {

				html += "<div class='col-md-3 py-3 py-md-0'>" + "<div class='card' id=" + produkt.produktId + ">"
						+ "<img src='../img/p6.png' alt=''>" + "<div class='card-body'>" + "<h3 class='text-center'>"
						+ produkt.produktName + "</h3>" + " <p class='text-center'>" + produkt.kategorie + "</p>" + "<h2>"
						+ produkt.preis + "&euro; <span>" + " <form action='ShoppingCartAppl.jsp'>"
						+ "<button name='btnProduktid' value='" + produkt.produktId
						+ "' type='submit' class='no-background-button'><li class='fa-solid fa-cart-shopping'></li></button></span></h2>"
						+ "</form>" + "</div>" + "</div>" + "</div>";
			}
		}else {
		...
		/*
		 * <div class='star text-center'> <i class="fa-solid fa-star checked"></i> <i
		 * class="fa-solid fa-star checked"></i> <i
		 * class="fa-solid fa-star checked"></i> <i
		 * class="fa-solid fa-star checked"></i> <i
		 * class="fa-solid fa-star checked"></i> </div>
		 */
	...
	public String getCategory() throws NoConnectionException, SQLException {
		...
		String sql = "select " + "kategorie " + "from bwi520_633040_634997.produkt";
		System.out.println(sql);
		ResultSet dbRes = new PostgreSQLAccess().getConnection().prepareStatement(sql).executeQuery();

		System.out.println(dbRes + "Category selected successfully");

		while (dbRes.next()) {

			if (!this.categoryList.contains(dbRes.getString("Kategorie"))) {
				this.categoryList.add(dbRes.getString("kategorie"));
		...
		for (String cat : categoryList) {
			...
			html += "<li><a class=\"dropdown-item\" href=\"IndexAppl.jsp?category="+cat+"\">" + cat + "</a></li>";
		}
	...
	public void getProduitCategorie(String categorie) throws NoConnectionException, SQLException {
		this.produktCategorieList.clear();
		...
		while (dbRes.next()) {
			this.produktCategorieList.add(new Produkt(dbRes.getInt("produktid"), 
